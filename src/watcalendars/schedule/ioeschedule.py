...
# --- Shared constants / mappings (same as WTC style) ---
...
ICS_HEADER = ["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//scheduleIOE//EN","CALSCALE:GREGORIAN"]
OUTPUT_DIR_NAME = "IOE_schedules"
...
def build_employee_code_index(employees_map:dict):
    ...
    for full in employees_map.values():
        ...
        base_first=_strip_diacritic(first)
        ...
        if td.has_attr('mergewith'):
            ...
            if code and len(code)<=6:
                ...
                if TITLE_KEYWORDS.search(full_plain):
                    tokens=full_plain.split(); cap=[t for t in tokens if NAME_TOKEN.match(t)]
...
# --- Parser (borrowed from WTC style) ---
...
def parse_wtc_table(html:str, employees_map:dict):
    ...
    lessons=[]; current_day=None; current_dates=[]
            ...
            for i,c in enumerate(cells[2:]):
                ...
                for line in raw_lines[1:]:
                    ...
                    if re.match(r'^[A-ZŻŹĆŁŚÓ][a-ząćęłńóśźżA-Z]{1,7}$', line) or re.match(r'^[A-Z][A-Za-z]{2,}$', line):
                        lecturer_codes.append(line); continue
...
# Alias for IOE
parse_ioe_table = parse_wtc_table

# Lecturer resolver (needs after alias definition for reference)
...
def resolve_lecturer_names(codes, variant_index, records, mode, legend_map=None):
    ...
    for raw in codes:
        ...
        if legend_full:
            resolved.append({'full':legend_full,'short':_strip_titles(legend_full)}); continue
        ...
        if not found and len(key)==2:
            ...
            if len(cand)==1: c=cand[0]; found={'full':c['full'],'short':c['short']}
        if not found and len(key)==2:
            for rec in records:
                parts=rec['short'].split()
                if len(parts)>=2 and key==_strip_diacritic(parts[0])[0].lower()+rec['surname'][0].lower():
                    found={'full':rec['full'],'short':rec['short']}; break
        if not found:
            for rec in records:
...
def save_all_ioe_calendars(schedules:dict):
    def do_all():
        logs=[]; saved=0
        ...
        return saved
    return log("Saving IOE schedules... ", do_all)

# --- Scrape orchestration ---
async def scrape_all_ioe(groups:list, base_pattern:str, employees:dict, concurrency:int=8):
    ...
    async with async_playwright() as p:
        ...
        async def worker(gid, idx, total):
            async with semaphore:
                ...
                results[gid]=parse_ioe_table(html, employees)
...
if __name__ == '__main__':
    ...
    employees=load_employees(); groups=load_groups('ioe')
    if not groups:
        print(f"{E} No IOE groups found."); sys.exit(1)
    schedules=run_async("Scraping IOE schedules... ", scrape_all_ioe, groups, url_pattern, employees, 8)
    saved=save_all_ioe_calendars(schedules)
