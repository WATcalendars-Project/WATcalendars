...
ICS_HEADER=["BEGIN:VCALENDAR","VERSION:2.0","PRODID:-//scheduleWIM//EN","CALSCALE:GREGORIAN"]
OUTPUT_DIR_NAME="WIM_schedules"
    ...
    for raw_line in full_text.split('\n'):
        ...
        if '  ' in line:
            ...
            if len(parts)==2:
                ...
                if abbr and full and len(full)>len(abbr)+3 and ' ' in full and re.match(r'^[A-Za-zĄąĆćĘęŁłŃńÓóŚśŹźŻż]{1,15}$', abbr): subject_map.setdefault(abbr,full); continue
        tokens=line.split()
        if 2<=len(tokens)<=12:
...
# Existing WTC-style parser (day+block row combined)
...
def parse_wtc_table(html:str, employees_map:dict):
        ...
        if first in DAY_ALIASES and len(cells)>2:
            ...
            for i,c in enumerate(cells[2:]):
                ...
                for line in raw_lines[1:]:
                    ...
                    if re.match(r'^[A-ZŻŹĆŁŚÓ][a-ząćęłńóśźżA-Z]{1,7}$', line) or re.match(r'^[A-Z][A-Za-z]{2,}$', line): lecturer_codes.append(line); continue
    ...
    # Post-process numbering & lecturers
    return _finalize_lessons(lessons)

# New WEL-style parser (day header row then multiple block rows only with block id)

def parse_wel_style_table(html:str, employees_map:dict):
    if not html: return []
    soup=BeautifulSoup(html,'html.parser')
    table=soup.find('table') or (soup.find_all('table')[0] if soup.find_all('table') else None)
    if not table: return []
    ...
    for tr in table.find_all('tr'):
        ...
        if not cells: continue
        first=cells[0].get_text(strip=True)
        # Day header row: day + empty (or non-block) cell + dates
        if first in DAY_ALIASES and len(cells)>2 and (len(cells)==0 or not cells[1].get_text(strip=True) or cells[1].get_text(strip=True) not in BLOCK_TIME_MAP):
            current_day=first; current_dates=[]
            for c in cells[2:]:
                token=c.get_text(strip=True).replace('\xa0',' ').strip()
                if not token: current_dates.append(None); continue
                m=DATE_TOKEN_RE.match(token)
            ...
            continue
        # Block rows: first cell is block
...
# Insert new rowspan detection and parser

def detect_rowspan_style(table)->bool:
    rows=table.find_all('tr')
    for tr in rows:
        cells=tr.find_all(['td','th'])
        if not cells or len(cells)<3: continue
        first_txt=cells[0].get_text(strip=True)
        if first_txt in DAY_ALIASES and ('rowspan' in cells[0].attrs or 'ROWSPAN' in cells[0].attrs):
            for c in cells[2:]:
                token=c.get_text(strip=True).replace('\xa0',' ').strip()
...
SUBJECT_TYPE_RE=re.compile(r'^(.+?)[\.;\s]*\(([A-Za-zĄąĆćĘęŁłŃńÓóŚśŹźŻż]{1,3})\)$')


def parse_rowspan_table(html:str, employees_map:dict):
    if not html: return []
    soup=BeautifulSoup(html,'html.parser')
    table=soup.find('table') or (soup.find_all('table')[0] if soup.find_all('table') else None)
    if not table: return []
    ...
    lecturer_legend=extract_lecturer_legend(soup)
    variant_index, employee_records = build_employee_code_index(employees_map)
    lessons=[]; current_day=None; date_index_map={}
    ...
    for tr in table.find_all('tr'):
        ...
        first_txt=cells[0].get_text(strip=True)
        if first_txt in DAY_ALIASES and (('rowspan' in cells[0].attrs) or ('ROWSPAN' in cells[0].attrs)):
            current_day=first_txt; date_index_map={}
            for idx,c in enumerate(cells):
                if idx<2: continue
                token=c.get_text(strip=True).replace('\xa0',' ').strip()
                if not token: date_index_map[idx]=None; continue
                m=DATE_TOKEN_RE.match(token)
                if m:
                    day_num=int(m.group(1)); roman=m.group(2); month=ROMAN_MONTH.get(roman)
                    if month:
                        try: date_index_map[idx]=datetime(base_year, month, day_num)
                        except ValueError: date_index_map[idx]=None
                    else: date_index_map[idx]=None
                else: date_index_map[idx]=None
            continue
        block_label=first_txt.strip()
        if current_day and block_label in BLOCK_TIME_MAP:
            start_time,end_time=BLOCK_TIME_MAP[block_label]
            for idx,c in enumerate(cells):
                if idx not in date_index_map: continue
                dt=date_index_map[idx]
                if not dt: continue
                raw_lines=[t.strip() for t in c.stripped_strings if t.strip() and t.strip()!='-']
                if not raw_lines: continue
                subject_token=raw_lines[0]
                symbol=''
                m_sub=SUBJECT_TYPE_RE.match(subject_token)
                if m_sub:
                    subject_abbr=m_sub.group(1).strip().rstrip('.')
                    symbol=m_sub.group(2).strip()
                else:
                    subject_abbr=subject_token.strip().rstrip('.')
                subject_key=re.sub(r'\(.*\)$','',subject_abbr).strip().rstrip('.')
                full_subject_name=subject_legend.get(subject_key, subject_legend.get(subject_abbr, subject_key))
                room=''; lecturer_codes=[]
                for line in raw_lines[1:]:
                    for part in [p.strip() for p in re.split(r'[;]', line) if p.strip()]:
                        if not room:
                            m_room=re.search(r'\b(\d{2,3}[A-Z]?)\b', part)
                            if m_room: room=m_room.group(1); continue
                        if re.match(r'^[A-Za-zŻŹĆŁŚÓąćęłńóśźż]{2,}[A-Za-z\.]*$', part) and len(part)<=10 and not part.isdigit():
                            lecturer_codes.append(part.replace('.', ''))
                type_full=TYPE_FULL_MAP.get(symbol, symbol)
                lecturers_full=resolve_lecturer_names(lecturer_codes, variant_index, employee_records, LECTURER_NAME_MODE, legend_map=lecturer_legend)
                lessons.append({'date': dt.strftime('%Y_%m_%d'), 'start': datetime.strptime(start_time,'%H:%M').replace(year=dt.year,month=dt.month,day=dt.day), 'end': datetime.strptime(end_time,'%H:%M').replace(year=dt.year,month=dt.month,day=dt.day), 'subject': subject_key, 'type': symbol, 'type_full': type_full, 'room': room, 'lesson_number': '', 'full_subject_name': full_subject_name, 'lecturers': lecturers_full})
    return _finalize_lessons(lessons)

# Auto detection wrapper

def detect_wel_style(table)->bool:
    rows=table.find_all('tr')
    for i,tr in enumerate(rows):
        cells=tr.find_all(['td','th'])
        if len(cells)>2 and cells[0].get_text(strip=True) in DAY_ALIASES and (not cells[1].get_text(strip=True)):
            # Look ahead for block-only row
            for j in range(i+1, min(i+5, len(rows))):
                c2=rows[j].find_all(['td','th'])
                if not c2: continue
                first=c2[0].get_text(strip=True)
                if first in BLOCK_TIME_MAP: return True
                if first in DAY_ALIASES and len(c2)>1 and c2[1].get_text(strip=True) in BLOCK_TIME_MAP: return False
    return False

# Replace original parse_wim_table with extended detection chain

def parse_wim_table(html:str, employees_map:dict):
    if not html: return []
    soup=BeautifulSoup(html,'html.parser')
    table=soup.find('table') or (soup.find_all('table')[0] if soup.find_all('table') else None)
    if not table: return []
    if detect_rowspan_style(table):
        return parse_rowspan_table(html, employees_map)
    if detect_wel_style(table):
        return parse_wel_style_table(html, employees_map)
    return parse_wtc_table(html, employees_map)

# Finalization helper

def _finalize_lessons(lessons):
    counter={}; totals={}
    for les in lessons:
...
def save_all_wim_calendars(schedules:dict):
    def do_all():
        logs=[]; saved=0
        ...
        return saved
    return log("Saving WIM schedules... ", do_all)

# Orchestration
async def scrape_all_wim(groups:list, base_pattern:str, employees:dict, concurrency:int=8):
    ...
    async with async_playwright() as p:
        ...
        async def worker(gid, idx, total):
            async with semaphore:
                ...
                results[gid]=parse_wim_table(html, employees)
...
if __name__=='__main__':
    start=time.time()
    print(f"{INFO} Start WIM schedules scraper {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    print(f"\n{INFO} Connection to WIM website with schedules.")
    url_pattern, desc = load_url_from_config(category='schedules', faculty='wim', url_type='url_lato')
    ...
    employees=load_employees(); groups=load_groups('wim')
    if not groups:
        print(f"{E} No WIM groups found."); sys.exit(1)
    schedules=run_async("Scraping WIM schedules... ", scrape_all_wim, groups, url_pattern, employees, 8)
    saved=save_all_wim_calendars(schedules)
