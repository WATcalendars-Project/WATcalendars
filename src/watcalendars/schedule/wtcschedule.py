...
# Flexible pattern: allow existing WTC* plus generic 2-5 letters + 2 digits + trailing alphanum
...
if __name__ == '__main__':
    ...
    url, description = load_url_from_config(category="groups", faculty="wtc", url_type="url_lato")
    ...
    try:
        ...
        if groups:
            ...
            print(f"{W} No WTC groups extracted.")
    except Exception as e:
        print(f"{E} {e}")

    duration = time.time() - start_time
    print(f"{INFO} WTC groups scraper finished {datetime.now().strftime('%Y-%m-%d %H:%M')}   duration: {duration:.2f}s")
