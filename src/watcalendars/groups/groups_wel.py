...
from watcalendars.utils.parsers.groups_parsers.groups_parser_wel import parse_wel_groups
...
if __name__ == '__main__':
    ...
    print(f"\n------[{datetime.now().strftime('%Y-%m-%d %H:%M')}] Start of WEL groups scraper:------\n")
    ...
    try:
        ...
        groups = parse_wel_groups(html, logs)
        print(f"{SUCCESS} Collected {len(groups)} WEL groups for {season}.")
        print("")
    ...
    print(f"{INFO} [{datetime.now().strftime('%Y-%m-%d %H:%M')}] WEL groups scraper finished  |  duration: {duration:.2f}s")
    print("")