...
def scrape_groups_wlo(url):
    print(f"\n{INFO} Scrape WLO groups (simple column mode).")
    logs = []
    ...
    groups = log("Scraping WLO groups list... ", log_scrape)
    print(f"{OK} Scraped {len(groups)} WLO groups (simple).")
    return groups
...
def save_to_file(groups):
    ...
    filename = os.path.join(db_dir, "wlo.txt")
    ...
    def save_log():
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            f.write('# WLO Groups list (simple column extraction)\n')
    ...
    try:
        new_count, total_count = log("Saving WLO groups file... ", save_log)
        if new_count:
            print(f"{OK} {new_count} new WLO groups (marked with [NEW]).")
        print(f"[INFO]: Total WLO groups: {total_count} in '{os.path.abspath(filename)}'.")
...
if __name__ == '__main__':
    ...
    print(f"{INFO} Start of WLO groups scraper {datetime.now().strftime('%Y-%m-%d %H:%M')}")
    ...
    try:
        ...
        if groups:
            ...
            print(f"{W} No WLO groups extracted.")
    except Exception as e:
        print(f"{E} {e}")

    duration = time.time() - start_time
    print(f"{INFO} WLO groups scraper finished {datetime.now().strftime('%Y-%m-%d %H:%M')}   duration: {duration:.2f}s")
