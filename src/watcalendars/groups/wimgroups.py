...
def scrape_groups_wim(url):
    print(f"\n{INFO} Scrape WIM groups (simple column mode).")
    logs = []
    ...
    groups = log("Scraping WIM groups list... ", log_scrape)
    print(f"{OK} Scraped {len(groups)} WIM groups (simple).")
    return groups
...
def save_to_file(groups):
    ...
    filename = os.path.join(db_dir, "wim.txt")
    ...
    def save_log():
        ...
        with open(filename, 'w', encoding='utf-8') as f:
            f.write('# WIM Groups list (simple column extraction)\n')
    ...
    try:
        new_count, total_count = log("Saving WIM groups file... ", save_log)
        if new_count:
            print(f"{OK} {new_count} new WIM groups (marked with [NEW]).")
        print(f"[INFO]: Total WIM groups: {total_count} in '{os.path.abspath(filename)}'.")
...
if __name__ == '__main__':
    ...
    print(f"{INFO} Start of WIM groups scraper {datetime.now().strftime('%Y-%m-%d %H:%M')}")

    print(f"\n{INFO} Connection to WIM website with groups.")

    url, description = load_url_from_config(category="groups", faculty="wim", url_type="url_lato")
    ...
    try:
        ...
        if groups:
            ...
            print(f"{W} No WIM groups extracted.")
    except Exception as e:
        print(f"{E} {e}")

    duration = time.time() - start_time
    print(f"{INFO} WIM groups scraper finished {datetime.now().strftime('%Y-%m-%d %H:%M')}   duration: {duration:.2f}s")
