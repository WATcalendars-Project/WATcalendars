...
from watcalendars.utils.parsers.groups_parsers.groups_parser_wcy import parse_wcy_groups
...
if __name__ == '__main__':
    ...
    print(f"\n------[{datetime.now().strftime('%Y-%m-%d %H:%M')}] Start of WCY groups scraper:------\n")
    ...
    try:
        print(f"{INFO} Scraping groups from URL: {url}")
        html, logs = scrape_html(url)
        print("")
        print(f"{INFO} Parsing {len(html)} bytes of HTML:")
        groups = parse_wcy_groups(html, logs)
        print(f"{SUCCESS} Collected {len(groups)} WCY groups.")
        print("")
    ...
    print(f"{INFO} [{datetime.now().strftime('%Y-%m-%d %H:%M')}] WCY groups scraper finished  |  duration: {duration:.2f}s")
    print("")