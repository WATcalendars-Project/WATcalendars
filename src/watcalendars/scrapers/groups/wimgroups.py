...
def scrape_groups_wim(url):
    ...
    def log_scrape_groups():
        ...
        log_entry("Parsing HTML content for WIM groups... Done.", logs)
        first_td = None
    ...
    groups = log("Scraping WIM groups names...", log_scrape_groups)
    print(f"{OK} Scraped {len(groups)} WIM groups.")
    return groups


def save_to_file(groups):
    filename = os.path.join(GROUPS_DIR, "wim.txt")
    if not os.path.exists(filename):
        print(f"Created a new db file for WIM groups: '{os.path.abspath(filename)}'")
    else:
        print(f"Using existing db file for WIM groups: '{os.path.abspath(filename)}'")
    ...
    try:
        new_count, total_count = log("Saving WIM groups to file... ", save_log)
        if new_count > 0:
            print(f"{OK} Summary: Saved {new_count} WIM groups (marked with [NEW]) in '{os.path.abspath(filename)}'.")
        else:
            print(f"{OK} Summary: No new WIM groups found since last run.")
        print(f"[INFO]: Total WIM groups in '{os.path.abspath(filename)}' ({total_count})")
    except Exception as e:
        print(f"{E} {e}")
...
if __name__ == '__main__':
    ...
    print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M')}] WIM groups scraper finished  |  duration: {duration:.2f}s")