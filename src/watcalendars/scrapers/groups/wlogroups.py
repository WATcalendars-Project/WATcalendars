...
def scrape_groups_wlo(url):
    ...
    def log_scrape_groups():
        ...
        log_entry("Parsing HTML content for WLO groups... Done.", logs)
    ...
    groups = log("Scraping WLO groups names...", log_scrape_groups)
    print(f"{OK} Scraped {len(groups)} WLO groups.")

    return groups


def save_to_file(groups):
    filename = os.path.join(GROUPS_DIR, "wlo.txt")

    if not os.path.exists(filename):
        print(f"Created a new db file for WLO groups: '{os.path.abspath(filename)}'")
    else:
        print(f"Using existing db file for WLO groups: '{os.path.abspath(filename)}'")
    ...
    try:
        new_count, total_count = log("Saving WLO groups to file... ", save_log)
        
        if new_count > 0:
            print(f"{OK} Summary: Saved {new_count} WLO groups (marked with [NEW]) in '{os.path.abspath(filename)}'.")

        else:
            print(f"{OK} Summary: No new WLO groups found since last run.")

        print(f"[INFO]: Total WLO groups in '{os.path.abspath(filename)}' ({total_count})")
