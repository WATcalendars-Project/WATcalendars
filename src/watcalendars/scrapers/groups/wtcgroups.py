...
def scrape_groups_wtc(url):
    ...
    def log_scrape_groups():
        ...
        log_entry("Parsing HTML content for WTC groups... Done.", logs)
    ...
    groups = log("Scraping WTC groups names...", log_scrape_groups)
...
def save_to_file(groups):
    filename = os.path.join(GROUPS_DIR, "wtc.txt")
    ...
    try:
        new_count, total_count = log("Saving WTC groups to file... ", save_log)

        if new_count > 0:
            print(f"{OK} Summary: Saved {new_count} WTC groups (marked with [NEW]) in '{os.path.abspath(filename)}'.")

        else:
            print(f"{OK} Summary: No new WTC groups found since last run.")

        print(f"[INFO]: Total WTC groups in '{os.path.abspath(filename)}' ({total_count})")
