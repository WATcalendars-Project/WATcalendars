...
async def main():
    ...
    print(f"\n------[{datetime.now().strftime('%Y-%m-%d %H:%M')}] Start WEL schedule scraper:------\n")
    ...
    html_map = await scrape_urls_async(
        pairs, 
        concurrency=10, 
        progress_label="Scraping groups for wel")
    print("")

    schedules = parse_schedules(html_map)
    print("")

    save_all_schedules(schedules, pairs, faculty_prefix="wel")
    print("")
    ...
    print(f"{INFO} [{datetime.now().strftime('%Y-%m-%d %H:%M')}] WEL schedules scraper finished (duration: {HH_MM_SS})")
    print("")
